  ...
  return (
    <section className="py-24 bg-white dark:bg-dark-bg transition-colors duration-300 overflow-hidden relative z-0">
      ...
      <motion.div
        ...
        className="absolute bottom-[-10%] right-[-5%] w-[500px] h-[500px] bg-accent-green/5 dark:bg-accent-green/10 rounded-full blur-[100px] pointer-events-none -z-10"
      />
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 gap-16 items-center relative z-10">
        ...
        >
          <motion.img
            ...
            className="absolute top-0 left-0 w-3/4 h-[450px] object-cover rounded-xl shadow-lg border-3 border-white dark:border-dark-bg z-10"
            animate={{ y: [0, -15, 0] }}
          ...
          <motion.img
            ...
            className="absolute bottom-0 right-0 w-2/3 h-[280px] object-cover rounded-xl shadow-2xl border-3 border-white dark:border-dark-bg z-20"
            animate={{ y: [0, 15, 0] }}
            transition={{ repeat: Infinity, duration: 5, ease: "easeInOut", delay: 1 }}
            whileHover={{ scale: 1.05, zIndex: 30 }}
          />
          {/* Badge Overlay */}
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-primary-green text-white w-30 h-30 rounded-full flex flex-col items-center justify-center p-4 shadow-xl border-2 border-white dark:border-dark-bg rotate-12 hover:rotate-0 transition-transform duration-500 z-40">
            <span className="text-3xl font-serif font-bold">5+</span>
            <span className="text-xs text-center font-semibold uppercase">Years of Experience</span>
          </div>
        </motion.div>
